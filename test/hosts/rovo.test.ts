...
import {
  ...
  doctorRovoInstructions,
  installRovoInstructions,
  uninstallRovoInstructions,
} from "../../src/index.js";
...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "tokenjuice-rovo-test-"));
  tempDirs.push(dir);
  return dir;
}

describe("rovo instructions", () => {
  it("installs a host-specific marker-delimited AGENTS.md instruction block", async () => {
    ...
    const result = await installRovoInstructions(instructionsPath);
    const instructions = await readFile(instructionsPath, "utf8");

    expect(result.instructionsPath).toBe(instructionsPath);
    expect(result.backupPath).toBeUndefined();
    expect(instructions).toContain("<!-- tokenjuice:rovo begin -->");
    expect(instructions).toContain("tokenjuice terminal output compaction");
    expect(instructions).toContain("When running terminal commands through Rovo Dev CLI");
  ...
  it("coexists with other tokenjuice AGENTS.md blocks", async () => {
    ...
    await installRovoInstructions(instructionsPath);
    const instructions = await readFile(instructionsPath, "utf8");
  ...
  it("backs up existing project instructions before replacing its own block", async () => {
    ...
    await expect(readFile(instructionsPath, "utf8")).resolves.toContain("<!-- tokenjuice:rovo begin -->");
  });
  ...
  it("reports installed and uninstalled instruction health", async () => {
    ...
    await installRovoInstructions(instructionsPath);
    const installed = await doctorRovoInstructions(instructionsPath);

    expect(installed.status).toBe("ok");
    expect(installed.advisories[0]).toContain("instruction-based");

    const removed = await uninstallRovoInstructions(instructionsPath);
    const disabled = await doctorRovoInstructions(instructionsPath);
  ...
  it("reports broken instructions with unmatched Rovo tokenjuice markers", async () => {
    const home = await createTempDir();
    const instructionsPath = join(home, "AGENTS.md");
    await writeFile(instructionsPath, "<!-- tokenjuice:rovo begin -->\nmissing end marker\n", "utf8");

    const doctor = await doctorRovoInstructions(instructionsPath);
  ...
  it("reports broken instructions with nested Rovo tokenjuice markers", async () => {
    const home = await createTempDir();
    const instructionsPath = join(home, "AGENTS.md");
    await writeFile(
      instructionsPath,
      [
        "<!-- tokenjuice:rovo begin -->",
        "<!-- tokenjuice:rovo begin -->",
        "## tokenjuice terminal output compaction",
        "",
        "- When running terminal commands through Rovo Dev CLI, prefer `tokenjuice wrap -- <command>`.",
    ...
    expect(doctor.status).toBe("broken");
    expect(doctor.issues).toContain("configured Rovo instructions have malformed tokenjuice markers");
    expect(doctor.fixCommand).toContain("remove unmatched tokenjuice markers");
    await expect(installRovoInstructions(instructionsPath)).rejects.toThrow("cannot safely repair malformed tokenjuice markers");
    await expect(uninstallRovoInstructions(instructionsPath)).rejects.toThrow(
      "cannot safely uninstall malformed tokenjuice markers",
    );
  });

  it("uses ROVO_DEV_PROJECT_DIR for the default AGENTS.md path", async () => {
    const home = await createTempDir();
    process.env.ROVO_DEV_PROJECT_DIR = home;

    const installed = await installRovoInstructions();
    const expectedInstructionsPath = join(home, "AGENTS.md");
    const doctor = await doctorRovoInstructions();
  ...
  it("defaults to the git root AGENTS.md from nested directories", async () => {
    ...
    await expect(readFile(join(root, "AGENTS.md"), "utf8")).resolves.toContain("Rovo Dev CLI");
  });
  ...
  it("is included in aggregate hook doctor output", async () => {
    ...
    await installRovoInstructions(undefined, { projectDir: home });

    const report = await doctorInstalledHooks({ projectDir: home });

    expect(report.integrations["rovo"].instructionsPath).toBe(join(home, "AGENTS.md"));
    expect(report.integrations["rovo"].status).toBe("ok");
  });
});
