...
import {
  doctorCodegenInstructions,
  doctorInstalledHooks,
  installCodegenInstructions,
  uninstallCodegenInstructions,
} from "../../src/index.js";
...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "tokenjuice-codegen-test-"));
  tempDirs.push(dir);
  return dir;
}

describe("Codegen instructions", () => {
  function countTokenjuiceBlocks(text: string): number {
    return text.match(/<!-- tokenjuice:codegen begin -->/gu)?.length ?? 0;
  }
  ...
  it("installs a host-specific marker-delimited AGENTS.md instruction block", async () => {
    ...
    const result = await installCodegenInstructions(instructionsPath);
  ...
  it("coexists with other tokenjuice AGENTS.md blocks", async () => {
    ...
    await installCodegenInstructions(instructionsPath);
    const instructions = await readFile(instructionsPath, "utf8");
  ...
  it("reports installed and uninstalled instruction health", async () => {
    ...
    await installCodegenInstructions(instructionsPath);
    const installed = await doctorCodegenInstructions(instructionsPath);

    expect(installed.status).toBe("ok");
    expect(installed.advisories[0]).toContain("rule-file based");

    const removed = await uninstallCodegenInstructions(instructionsPath);
    const disabled = await doctorCodegenInstructions(instructionsPath);
  ...
  it("reports broken instructions with unmatched tokenjuice markers", async () => {
    const home = await createTempDir();
    const instructionsPath = join(home, "AGENTS.md");
    await writeFile(instructionsPath, "<!-- tokenjuice:codegen begin -->\nmissing end marker\n", "utf8");

    const doctor = await doctorCodegenInstructions(instructionsPath);
  ...
  it("reports broken instructions with nested Codegen tokenjuice markers", async () => {
    const home = await createTempDir();
    const instructionsPath = join(home, "AGENTS.md");
    await writeFile(
      instructionsPath,
    ...
    expect(doctor.status).toBe("broken");
    expect(doctor.issues).toContain("configured Codegen instructions have malformed tokenjuice markers");
    expect(doctor.fixCommand).toContain("remove unmatched tokenjuice markers");
    await expect(installCodegenInstructions(instructionsPath)).rejects.toThrow("cannot safely repair malformed tokenjuice markers");
    await expect(uninstallCodegenInstructions(instructionsPath)).rejects.toThrow("cannot safely uninstall malformed tokenjuice markers");
  });
  ...
  it("uses CODEGEN_PROJECT_DIR for the default AGENTS.md path", async () => {
    const home = await createTempDir();
    process.env.CODEGEN_PROJECT_DIR = home;

    const installed = await installCodegenInstructions();
    const expectedInstructionsPath = join(home, "AGENTS.md");
    const doctor = await doctorCodegenInstructions();
  ...
  it("defaults to the git root AGENTS.md from nested directories", async () => {
    ...
    await expect(readFile(join(root, "AGENTS.md"), "utf8")).resolves.toContain("Codegen agents");
  });

  it("reports codegen in aggregate hook doctor", async () => {
    const home = await createTempDir();
    for (const key of envKeys) {
      process.env[key] = home;
    }

    await installCodegenInstructions(undefined, { projectDir: home });
    const report = await doctorInstalledHooks({ projectDir: home });

    expect(report.integrations.codegen.instructionsPath).toBe(join(home, "AGENTS.md"));
    expect(report.integrations.codegen.status).toBe("ok");
  });
  ...
  it("removes the default AGENTS.md when only tokenjuice content remains", async () => {
    ...
    await installCodegenInstructions();
    await uninstallCodegenInstructions(instructionsPath);
