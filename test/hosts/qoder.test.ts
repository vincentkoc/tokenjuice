...
import {
  ...
  doctorQoderInstructions,
  installQoderInstructions,
  uninstallQoderInstructions,
} from "../../src/index.js";
...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "tokenjuice-qoder-test-"));
  tempDirs.push(dir);
  return dir;
}

describe("qoder instructions", () => {
  it("installs a host-specific marker-delimited AGENTS.md instruction block", async () => {
    ...
    const result = await installQoderInstructions(instructionsPath);
  ...
  it("coexists with other tokenjuice AGENTS.md blocks", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "- When running terminal commands through Grok Build, prefer `tokenjuice wrap -- <command>`.",
        "<!-- tokenjuice:grok-build end -->",
      ].join("\n"),
      "utf8",
    );

    await installQoderInstructions(instructionsPath);
    const instructions = await readFile(instructionsPath, "utf8");
  ...
  it("backs up existing project instructions before replacing its own block", async () => {
    ...
    await expect(readFile(instructionsPath, "utf8")).resolves.toContain("<!-- tokenjuice:qoder begin -->");
  });
  ...
  it("reports installed and uninstalled instruction health", async () => {
    ...
    await installQoderInstructions(instructionsPath);
    const installed = await doctorQoderInstructions(instructionsPath);

    expect(installed.status).toBe("ok");
    expect(installed.advisories[0]).toContain("instruction-based");

    const removed = await uninstallQoderInstructions(instructionsPath);
    const disabled = await doctorQoderInstructions(instructionsPath);
  ...
  it("reports broken instructions with unmatched Qoder tokenjuice markers", async () => {
    const home = await createTempDir();
    const instructionsPath = join(home, "AGENTS.md");
    await writeFile(instructionsPath, "<!-- tokenjuice:qoder begin -->\nmissing end marker\n", "utf8");

    const doctor = await doctorQoderInstructions(instructionsPath);
  ...
  it("reports broken instructions when a valid block has extra tokenjuice markers", async () => {
    ...
    expect(doctor.status).toBe("broken");
    expect(doctor.issues).toContain("configured Qoder instructions have unmatched tokenjuice markers");
    expect(doctor.fixCommand).toContain("remove unmatched tokenjuice markers");
    await expect(installQoderInstructions(instructionsPath)).rejects.toThrow(
      "cannot safely repair malformed tokenjuice markers",
    );
    await expect(uninstallQoderInstructions(instructionsPath)).rejects.toThrow(
      "cannot safely uninstall malformed tokenjuice markers",
    );
  });

  it("uses QODER_PROJECT_DIR for the default AGENTS.md path", async () => {
    const home = await createTempDir();
    process.env.QODER_PROJECT_DIR = home;

    const installed = await installQoderInstructions();
    const expectedInstructionsPath = join(home, "AGENTS.md");
    const doctor = await doctorQoderInstructions();
  ...
  it("defaults to the git root AGENTS.md from nested directories", async () => {
    ...
    await expect(readFile(join(root, "AGENTS.md"), "utf8")).resolves.toContain("Qoder CLI");
  });
  ...
  it("is included in aggregate hook doctor output", async () => {
    ...
    await installQoderInstructions(undefined, { projectDir: home });

    const report = await doctorInstalledHooks({ projectDir: home });

    expect(report.integrations.qoder.instructionsPath).toBe(join(home, "AGENTS.md"));
    expect(report.integrations.qoder.status).toBe("ok");
  });
  ...
  it("uses projectDir when uninstalling the default instructions file", async () => {
    ...
    await installQoderInstructions(undefined, { projectDir: home });
    const removed = await uninstallQoderInstructions(undefined, { projectDir: home });
