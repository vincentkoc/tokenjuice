...
import {
  doctorStagewiseSkill,
  doctorInstalledHooks,
  installStagewiseSkill,
  uninstallStagewiseSkill,
} from "../../src/index.js";
...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "tokenjuice-stagewise-test-"));
  const realDir = await realpath(dir);
  tempDirs.push(realDir);
  return realDir;
}

describe("Stagewise skill", () => {
  it("installs a workspace skill with Stagewise-compatible frontmatter", async () => {
    const home = await createTempDir();
    const skillPath = join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md");

    const result = await installStagewiseSkill(skillPath, { projectDir: home });
  ...
  it("backs up an existing skill before replacing it", async () => {
    const home = await createTempDir();
    const skillPath = join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md");
    await installStagewiseSkill(skillPath, { projectDir: home });
  ...
  it("reports installed and uninstalled skill health", async () => {
    ...
    await installStagewiseSkill(skillPath, { projectDir: home });
    const installed = await doctorStagewiseSkill(skillPath, { projectDir: home });
    ...
    const removed = await uninstallStagewiseSkill(skillPath, { projectDir: home });
    const disabled = await doctorStagewiseSkill(skillPath, { projectDir: home });
  ...
  it("preserves user-owned tokenjuice skill files on uninstall", async () => {
    const home = await createTempDir();
    const skillPath = join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md");
    await mkdir(join(home, ".stagewise", "skills", "tokenjuice"), { recursive: true });
    await writeFile(skillPath, "# custom tokenjuice skill\n\nkeep me\n", "utf8");

    const removed = await uninstallStagewiseSkill(undefined, { projectDir: home });
  ...
  it("reports broken skills when required tokenjuice guidance is stale", async () => {
    ...
    const doctor = await doctorStagewiseSkill(skillPath, { projectDir: home });

    expect(doctor.status).toBe("broken");
    expect(doctor.hasTokenjuiceMarker).toBe(true);
    expect(doctor.issues).toContain("configured Stagewise skill is missing discovery frontmatter");
    expect(doctor.issues).toContain("configured Stagewise skill is missing the raw escape hatch");
    expect(doctor.issues).toContain("configured Stagewise skill is missing workspace skill path guidance");
    expect(doctor.issues).toContain("configured Stagewise skill still suggests the full escape hatch");
  });
  ...
  it("requires name and description in top-of-file frontmatter", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "- Stagewise discovers this reusable skill from `.stagewise/skills/tokenjuice/SKILL.md`.",
      ].join("\n"),
      "utf8",
    );
    ...
    expect(doctor.issues).toContain("configured Stagewise skill is missing the tokenjuice ownership marker");
    expect(doctor.issues).toContain("configured Stagewise skill is missing the required tokenjuice skill name");
    expect(doctor.issues).toContain("configured Stagewise skill is missing discovery frontmatter");
  });
  ...
  it("accepts quoted YAML skill names in top-of-file frontmatter", async () => {
    ...
    await writeFile(
      ...
      [
        "---",
        'name: "tokenjuice" # generated by Stagewise',
  ...
  it("rejects malformed YAML frontmatter even when guidance is present", async () => {
    const home = await createTempDir();
    const skillPath = join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md");
    await mkdir(join(home, ".stagewise", "skills", "tokenjuice"), { recursive: true });
    await writeFile(
    ...
    expect(doctor.status).toBe("broken");
    expect(doctor.issues).toContain("configured Stagewise skill has invalid discovery frontmatter");
  });
  ...
  it.each([
    ...
    "rejects invalid simple YAML scalar frontmatter: %s",
    async (descriptionLine) => {
      const home = await createTempDir();
      const skillPath = join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md");
      await mkdir(join(home, ".stagewise", "skills", "tokenjuice"), { recursive: true });
      await writeFile(
  ...
  it("rejects duplicate frontmatter keys before deriving skill health", async () => {
    const home = await createTempDir();
    const skillPath = join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md");
    await mkdir(join(home, ".stagewise", "skills", "tokenjuice"), { recursive: true });
    await writeFile(
  ...
  it("uses STAGEWISE_PROJECT_DIR for the default skill path", async () => {
    const home = await createTempDir();
    process.env.STAGEWISE_PROJECT_DIR = home;

    const installed = await installStagewiseSkill();
    const expectedSkillPath = join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md");
    const doctor = await doctorStagewiseSkill();
  ...
  it("rejects symlinked skill files before reading or backing them up", async () => {
    ...
    await symlink(join(outside, "private.md"), join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md"));

    await expect(installStagewiseSkill()).rejects.toThrow(/will not read or write through instruction symlinks/u);
    await expect(access(join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md.bak"))).rejects.toMatchObject({
      code: "ENOENT",
    });
    ...
    await expect(
      doctorStagewiseSkill(join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md"), { projectDir: home }),
  ...
  it("rejects explicit paths outside the project", async () => {
    ...
    await expect(
      installStagewiseSkill(join(outside, ".stagewise", "skills", "tokenjuice", "SKILL.md"), { projectDir: home }),
  ...
  it("rejects explicit non-default project skill paths", async () => {
    ...
    await expect(
      installStagewiseSkill(join(home, ".stagewise", "skills", "other", "SKILL.md"), { projectDir: home }),
    ).rejects.toThrow("project-local .stagewise/skills/tokenjuice/SKILL.md");
  });
  ...
  it("rejects symlinked project roots", async () => {
    ...
    await expect(installStagewiseSkill(undefined, { projectDir: link })).rejects.toThrow("instruction symlinks");
  });
  ...
  it("canonicalizes symlinked project ancestors before writing", async () => {
    ...
    const installed = await installStagewiseSkill(undefined, { projectDir: join(linkParent, "project") });

    expect(installed.skillPath).toBe(join(project, ".stagewise", "skills", "tokenjuice", "SKILL.md"));
  ...
  it("accepts explicit skill paths through symlinked project ancestors", async () => {
    ...
    const linkedSkillPath = join(linkedProject, ".stagewise", "skills", "tokenjuice", "SKILL.md");

    const installed = await installStagewiseSkill(linkedSkillPath, { projectDir: linkedProject });
  ...
  it("rejects symlinked skill path components", async () => {
    ...
    await symlink(outside, join(home, ".stagewise"));

    await expect(installStagewiseSkill(undefined, { projectDir: home })).rejects.toThrow("instruction symlinks");
  });
  ...
  it("does not fail doctors for missing default skills under symlinked roots", async () => {
    ...
    const direct = await doctorStagewiseSkill(undefined, { projectDir: link });
    ...
    expect(aggregate.integrations["stagewise"].status).toBe("disabled");
    expect(aggregate.integrations["stagewise"].hasTokenjuiceMarker).toBe(false);
    expect(aggregate.integrations["stagewise"].hasUnsafePathIssue).toBe(false);
  });
  ...
  it("rejects sibling temp or backup symlinks", async () => {
    ...
    await expect(installStagewiseSkill(skillPath, { projectDir: home })).rejects.toThrow(
  ...
  it("surfaces unsafe default skills in aggregate doctor without reading them", async () => {
    ...
    await mkdir(join(home, ".stagewise", "skills", "tokenjuice"), { recursive: true });
    await writeFile(join(outside, "shared-skill.md"), "# shared Stagewise skill\n", "utf8");
    await symlink(join(outside, "shared-skill.md"), join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md"));

    const report = await doctorInstalledHooks();

    expect(report.integrations["stagewise"].status).toBe("broken");
    expect(report.integrations["stagewise"].hasUnsafePathIssue).toBe(true);
    expect(report.status).toBe("broken");
  });

  it("does not count user-owned Stagewise skill files as aggregate tokenjuice installs", async () => {
    const home = await createTempDir();
    for (const key of envKeys) {
      process.env[key] = home;
    }
    ...
    expect(report.integrations["stagewise"].status).toBe("broken");
    expect(report.integrations["stagewise"].hasTokenjuiceMarker).toBe(false);
  ...
  it("defaults to the git root skill from nested directories", async () => {
    ...
    expect(installed.skillPath).toBe(join(root, ".stagewise", "skills", "tokenjuice", "SKILL.md"));
  });
  ...
  it("is included in aggregate hook doctor output", async () => {
    ...
    await installStagewiseSkill(undefined, { projectDir: home });

    const report = await doctorInstalledHooks();

    expect(report.integrations["stagewise"].status).toBe("ok");
    expect(report.integrations["stagewise"].skillPath).toBe(join(home, ".stagewise", "skills", "tokenjuice", "SKILL.md"));
  });
});
